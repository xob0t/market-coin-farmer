...
<template>
  <div :class="twMerge('bg-background flex select-none flex-row items-center justify-between overflow-hidden', $attrs.class as string)" style="--wails-draggable: drag">
    <slot />
    <div style="--wails-draggable: none">
      <WindowControls v-bind="customProps('ml-auto')" />
    </div>
  </div>
</template>
